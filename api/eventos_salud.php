...
function getEventosSalud($conn) {
    $result = $conn->query("SELECT * FROM eventos_salud");
    if (!$result) {
        http_response_code(500);
...
function addEventoSalud($conn) {
    if (!isset($_POST['nombre_evento'], $_POST['descripcion'], $_POST['fecha'], $_POST['acciones'])) {
    ...
    $stmt->bind_param("ssss", $_POST['nombre_evento'], $_POST['descripcion'], $_POST['fecha'], $_POST['acciones']);
    if (!$stmt->execute()) {
        http_response_code(500);
